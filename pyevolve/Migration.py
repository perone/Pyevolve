...
class WANMigration(MigrationScheme):
   ...
   if you want to change the default selector, you must do this: ::
    ...
    def __init__(self, host, port, group_name):
        ...
        self.clientThread = Network.UDPThreadUnicastClient(
            self.myself[0], rand_randint(30000, 65534))
...
class MPIMigration(MigrationScheme):
    ...
    def __init__(self):
        ...
        if not HAS_MPI4PY:
            raise ImportError(
                "No module named mpi4py, you must install mpi4py to use MPIMIgration!")
