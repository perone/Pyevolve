        ...
        for it in xrange(stringLength):
            if Util.randomFlipCoin(args["pmut"]):
                Util.listSwapElement(
                    genome, it, rand_randint(0, stringLength - 1))
                mutations += 1
...
def G1DListMutatorSwap(genome, **args):
    ...
    if mutations < 1.0:
        ...
        for it in xrange(int(round(mutations))):
            Util.listSwapElement(genome, rand_randint(
                0, listSize - 1), rand_randint(0, listSize - 1))
...
def G1DListMutatorAllele(genome, **args):
    ...
    if allele is None:
        Util.raiseException(
            "to use the G1DListMutatorAllele, you must specify the 'allele' parameter", TypeError)
        ...
        for i in xrange(height):
            for j in xrange(width):
                if Util.randomFlipCoin(args["pmut"]):
                    index_b = (rand_randint(0, height - 1),
                               rand_randint(0, width - 1))
                    Util.list2DSwapElement(
                        genome.genomeString, (i, j), index_b)
                    mutations += 1
    else:
        for it in xrange(int(round(mutations))):
            index_a = (rand_randint(0, height - 1), rand_randint(0, width - 1))
