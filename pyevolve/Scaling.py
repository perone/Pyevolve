...
def LinearScaling(pop):
    ...
    for i in xrange(len(pop)):
        ...
        if f < 0.0:
            Util.raiseException(
                "Score %r is negative, linear scaling not supported !" % (f,), ValueError)
        f = f * a + b
        if f < 0:
            f = 0.0
        pop[i].fitness = f
...
def PowerLawScaling(pop):
    ...
    for i in xrange(len(pop)):
        ...
        if f < 0.0:
            Util.raiseException(
                "Score %r is negative, power law scaling not supported !" % (f,), ValueError)
        f = math.pow(f, k)
        pop[i].fitness = f
...
def BoltzmannScaling(pop):
    ...
    boltz_temperature = pop.getParam(
        "boltz_temperature", Consts.CDefScaleBoltzStart)
