...
class UDPThreadBroadcastClient(threading.Thread):
    ...
    def run(self):
        ...
        if self.data is None:
            Util.raiseException(
                'You must set the data with setData method', ValueError)
...
class UDPThreadUnicastClient(threading.Thread):
    ...
    def addData(self, data):
        ...
        if self.poolLength() >= self.poolSize:
            logging.warning(
                'the send pool is full, consider increasing the pool size or decreasing the timeout !')
            return
    ...
    def run(self):
        ...
        if len(self.target) <= 0:
            Util.raiseException(
                'You must set the target(s) before send data', ValueError)
