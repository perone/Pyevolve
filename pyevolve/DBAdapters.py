...
class DBFileCSV(DBBaseAdapter):
    ...
    def __init__(
            self,
            filename=Consts.CDefCSVFileName,
            identify=None,
            frequency=Consts.CDefCSVFileStatsGenFreq,
            reset=True
    ):
...
class DBSQLite(DBBaseAdapter):
    ...
    def __init__(
            self,
            dbname=Consts.CDefSQLiteDBName,
            identify=None,
            resetDB=False,
            resetIdentify=True,
            frequency=Consts.CDefSQLiteStatsGenFreq,
    ...
    def insert(self, ga_engine):
        ...
        if not generation % self.commitFreq:
            self.commit()
...
class DBVPythonGraph(DBBaseAdapter):
    ...
    def makeDisplay(self, title_sec, x, y, ga_engine):
        ...
        if self.genmax:
            disp = self.vtkGraph.gdisplay(
                title=title,
                xtitle='Generation',
                ytitle=title_sec,
                xmax=ga_engine.getGenerations(),
                xmin=0.,
                width=500,
                height=250,
                x=x,
...
class DBMySQLAdapter(DBBaseAdapter):
    ...
    def open(self, ga_engine):
        ...
        self.connection = self.mysqldbmod.connect(
            host=self.host,
            user=self.user,
            passwd=self.passwd,
            db=self.db,
