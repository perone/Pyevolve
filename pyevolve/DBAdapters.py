...
class DBBaseAdapter(object):
    ...
    def __init__(self, frequency, identify):
        ...
        if identify is None:
            self.identify = datetime.datetime.strftime(
    ...
    def insert(self, ga_engine):
        ...
        """
        Util.raiseException(
            "This method is not implemented on the ABC", NotImplementedError)
...
class DBFileCSV(DBBaseAdapter):
    ...
    def __repr__(self):
        ...
        ret = "DBFileCSV DB Adapter [File='%s', identify='%s']" % (
            self.filename, self.getIdentify())
        return ret
    ...
    def open(self, ga_engine):
        ...
        logging.debug(
            "Opening the CSV file to dump statistics [%s]", self.filename)
...
class DBURLPost(DBBaseAdapter):
    ...
    **identify**
    ...
    def __repr__(self):
        """ The string representation of adapter """
        ret = "DBURLPost DB Adapter [URL='%s', identify='%s']" % (
            self.url, self.getIdentify())
        return ret
    ...
    def insert(self, ga_engine):
        ...
        if self.post:  # POST
            response = self.urllibmod.urlopen(
                self.url, self.urllibmod.urlencode(params))
        else:  # GET
            response = self.urllibmod.urlopen(
                self.url + "?%s" % (self.urllibmod.urlencode(params)))
        if response:
            response.close()
...
class DBSQLite(DBBaseAdapter):
    ...
    def __repr__(self):
        """ The string representation of adapter """
        ret = "DBSQLite DB Adapter [File='%s', identify='%s']" % (
            self.dbName, self.getIdentify())
        return ret
    ...
    def createStructure(self, stats):
        ...
        pstmt = pstmt[:-2] + ")"
        logging.debug("Creating table %s: %s.",
                      Consts.CDefSQLiteDBTable, pstmt)
        c.execute(pstmt)

        pstmt = """create table if not exists %s(identify text, generation integer,
              individual integer, fitness real, raw real)""" % (Consts.CDefSQLiteDBTablePop)
        logging.debug("Creating table %s: %s.",
                      Consts.CDefSQLiteDBTablePop, pstmt)
        c.execute(pstmt)
        self.commit()
    ...
    def resetTableIdentify(self):
        ...
        stmt2 = "delete from %s where identify = ?" % (
            Consts.CDefSQLiteDBTablePop)

        logging.debug(
            "Erasing data from the tables with the identify = %s", self.getIdentify())
        try:
    ...
    def resetStructure(self, stats):
        ...
        """
        logging.debug(
            "Reseting structure, droping table and creating new empty table.")
        c = self.getCursor()
    ...
    def insert(self, ga_engine):
        ...
        pstmt = "insert into %s values(?, ?, ?, ?, ?)" % (
            Consts.CDefSQLiteDBTablePop,)
        tups = []
        for i in xrange(len(population)):
            ind = population[i]
            tups.append((self.getIdentify(), generation,
                         i, ind.fitness, ind.score))
...
class DBXMLRPC(DBBaseAdapter):
    ...
    def __repr__(self):
        """ The string representation of adapter """
        ret = "DBXMLRPC DB Adapter [URL='%s', identify='%s']" % (
            self.url, self.getIdentify())
        return ret
...
class DBVPythonGraph(DBBaseAdapter):
    ...
    def makeDisplay(self, title_sec, x, y, ga_engine):
        ...
        """
        title = "Pyevolve v.%s - %s - id [%s]" % (
            __version__, title_sec, self.identify)
        if self.genmax:
    ...
    def open(self, ga_engine):
        ...
        display_rawdev = self.makeDisplay(
            "Raw Score (std. dev.)", 500, 0, ga_engine)
        display_rawavg = self.makeDisplay(
            "Raw Score (avg)", 500, 250, ga_engine)

        self.curveMin = self.vtkGraph.gcurve(
            color=self.vtkGraph.color.red, gdisplay=display_rawmin)
        self.curveMax = self.vtkGraph.gcurve(
            color=self.vtkGraph.color.green, gdisplay=display_rawmax)
        self.curveDev = self.vtkGraph.gcurve(
            color=self.vtkGraph.color.blue, gdisplay=display_rawdev)
        self.curveAvg = self.vtkGraph.gcurve(
            color=self.vtkGraph.color.orange, gdisplay=display_rawavg)
...
class DBMySQLAdapter(DBBaseAdapter):
    ...
    def createStructure(self, stats):
        ...
        logging.debug("Creating table %s: %s.",
                      Consts.CDefMySQLDBTablePop, pstmt)
        c.execute(pstmt)
        self.commit()
    ...
    def resetTableIdentify(self):
        """ Delete all records on the table with the same Identify """
        c = self.getCursor()
        stmt = "delete from %s where identify = '%s'" % (
            Consts.CDefMySQLDBTable, self.getIdentify())
        stmt2 = "delete from %s where identify = '%s'" % (
            Consts.CDefMySQLDBTablePop, self.getIdentify())
