...
def G1DBinaryStringXTwoPoint(genome, **args):
    ...
    if len(gMom) == 1:
        Util.raiseException(
            "The Binary String have one element, can't use the Two Point Crossover method !", TypeError)
...
def G1DListCrossoverSinglePoint(genome, **args):
    ...
    if len(gMom) == 1:
        Util.raiseException(
            "The 1D List have one element, can't use the Single Point Crossover method !", TypeError)
...
def G1DListCrossoverTwoPoint(genome, **args):
    ...
    if len(gMom) == 1:
        Util.raiseException(
            "The 1D List have one element, can't use the Two Point Crossover method !", TypeError)
...
def G1DListCrossoverOX(genome, **args):
    ...
    if args["count"] >= 1:
        ...
        sister.genomeList = P1[listSize - c2:] + \
            gDad[c1:c2] + P1[:listSize - c2]

    if args["count"] == 2:
        brother = gDad.clone()
        brother.resetStats()
        P2 = [c for c in gDad[c2:] + gDad[:c2] if c not in gMom[c1:c2]]
        brother.genomeList = P2[listSize - c2:] + \
            gMom[c1:c2] + P2[:listSize - c2]
...
def G1DListCrossoverEdge(genome, **args):
    ...
    mom_edges, dad_edges, merge_edges = Util.G1DListGetEdgesComposite(
        gMom, gDad)
...
def G1DListCrossoverRealSBX(genome, **args):
    ...
    for i in range(0, len(gMom)):
        if math.fabs(gMom[i] - gDad[i]) > EPS:
            ...
            brother[i] = 0.5 * ((gMom[i] + gDad[i]) -
                                beta_q * (gDad[i] - gMom[i]))
