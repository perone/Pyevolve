...
def GTournamentSelector(population, **args):
    ...
    poolSize = population.getParam(
        "tournamentPool", Consts.CDefTournamentPoolSize)
    tournament_pool = [GRouletteWheel(population, **args)
                       for i in xrange(poolSize)]

    if population.sortType == Consts.sortType["scaled"]:
        choosen = minimax_operator(
            tournament_pool, key=lambda ind: ind.fitness)
    else:
...
def GTournamentSelectorAlternative(population, **args):
    ...
    """
    pool_size = population.getParam(
        "tournamentPool", Consts.CDefTournamentPoolSize)
    len_pop = len(population)
    should_minimize = population.minimax == Consts.minimaxType["minimize"]
    minimax_operator = min if should_minimize else max
    tournament_pool = [population[random.randint(
        0, len_pop - 1)] for i in xrange(pool_size)]
    ...
    if population.sortType == Consts.sortType["scaled"]:
        ...
        if pop_fitMax == pop_fitMin:
            ...
            if population.minimax == Consts.minimaxType["maximize"]:
                ...
                for i in xrange(1, len_pop):
                    psum[i] = -population[i].fitness + \
                        pop_fitMax + pop_fitMin + psum[i - 1]
                for i in xrange(len_pop):
                    psum[i] /= float(psum[len_pop - 1])
    else:
        ...
        if pop_rawMax == pop_rawMin:
            ...
            if population.minimax == Consts.minimaxType["maximize"]:
                ...
                for i in xrange(1, len_pop):
                    psum[i] = - population[i].score + \
                        pop_rawMax + pop_rawMin + psum[i - 1]
                for i in xrange(len_pop):
                    psum[i] /= float(psum[len_pop - 1])
