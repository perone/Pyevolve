...
class GenomeBase(object):
    ...
    __slots__ = ["evaluator", "initializator", "mutator",
                 "crossover", "internalParams", "score", "fitness"]
...
class GTreeNodeBase(object):
    ...
    def __init__(self, parent, childs=None):
        ...
        if childs is not None:
            if type(childs) != list:
                Util.raiseException(
                    "Childs must be a list of nodes", TypeError)
            typecheck_list = filter(
                lambda x: not isinstance(x, GTreeNodeBase), childs)
...
class GTreeBase(GenomeBase):
    ...
    """
    __slots__ = ["root_node", "tree_height",
                 "nodes_list", "nodes_leaf", "nodes_branch"]
    ...
    def processNodes(self, cloning=False):
        ...
        self.nodes_branch = filter(
            lambda n: n.isLeaf() is False, self.nodes_list)
    ...
    def traversal(self, callback, start_node=None):
        ...
        if not inspect.isfunction(callback):
            Util.raiseException(
                "The callback for the tree traversal must be a function", TypeError)
