...
def graph_pop_heatmap_raw(pop, minimize, colormap="jet", filesave=None):
    pylab.imshow(pop, aspect="auto", interpolation="gaussian",
                 cmap=matplotlib.cm.__dict__[colormap])
...
def graph_pop_heatmap_fitness(pop, minimize, colormap="jet", filesave=None):
    pylab.imshow(pop, aspect="equal", interpolation="gaussian",
                 cmap=matplotlib.cm.__dict__[colormap])
...
def graph_maxmin_raw(pop, minimize, filesave=None):
    ...
    pylab.fill_between(x, min_y, max_y, color="g",
                       alpha=0.1, label="Diff max/min")
    ...
    pylab.title("Plot of evolution identified by '%s' (raw scores)" %
                (options.identify))
...
def graph_compare_raw(pop, minimize, id_list, filesave=None):
    ...
    for it_out in pop:
        ...
        if minimize:
            ...
            pylab.plot(x, min_y, colors_list[index], label="Raw min (%s)" % (
                id_list[index],), linewidth=1.3)
        else:
            pylab.plot(x, max_y, colors_list[index], label="Raw max (%s)" % (
                id_list[index],), linewidth=1.3)
            pylab.plot(x, min_y, colors_list[index], linewidth=0.05)

        pylab.fill_between(x, min_y, max_y, color=colors_list[
                           index], alpha=0.06,)
...
def graph_compare_fitness(pop, minimize, id_list, filesave=None):
    ...
    for it_out in pop:
        ...
        if minimize:
            pylab.plot(x, max_y, colors_list[index], linewidth=0.05)
            pylab.plot(x, min_y, colors_list[index], label="Fitness min (%s)" % (
                id_list[index],), linewidth=1.3)
        else:
            pylab.plot(x, max_y, colors_list[index], label="Fitness max (%s)" % (
                id_list[index],), linewidth=1.3)
            pylab.plot(x, min_y, colors_list[index], linewidth=0.05)
    ...
    pylab.ylabel("Fitness score")
    pylab.title(
        "Plot of evolution identified by '%s' (fitness scores)" % ('many',))
    pylab.grid(True)
    pylab.legend(prop=FontProperties(size="smaller"))
...
if __name__ == "__main__":
    ...
    group.add_option("-0", action="store_true",
                     help="Write all graphs to files. Graph types: 1, 2, 3, 4 and 5.", dest="all_graphs")

    group.add_option("-1", action="store_true",
                     help="Error bars graph (raw scores).", dest="errorbars_raw")
    group.add_option("-2", action="store_true",
                     help="Error bars graph (fitness scores).", dest="errorbars_fitness")
    group.add_option("-3", action="store_true",
                     help="Max/min/avg/std. dev. graph (raw scores).", dest="maxmin_raw")
    group.add_option("-4", action="store_true",
                     help="Max/min/avg graph (fitness scores).", dest="maxmin_fitness")
    group.add_option("-5", action="store_true",
                     help="Raw and Fitness min/max difference graph.", dest="diff_raw")
    ...
    group.add_option("-8", action="store_true",
                     help="Show a heat map of population raw score distribution between generations.", dest="pop_heatmap_raw")
    group.add_option("-9", action="store_true",
                     help="Show a heat map of population fitness score distribution between generations.", dest="pop_heatmap_fitness")
    ...
    if options.pop_heatmap_raw or options.pop_heatmap_fitness:
        ...
        if options.genrange:
            ...
            ret = c.execute("select distinct generation from population where identify = ? and generation between ? and ?",
                            (options.identify, genrange[0], genrange[1]))
        else:
            ret = c.execute(
                "select distinct generation from population where identify = ?", (options.identify,))
    ...
    if len(identify_list) == 1 and not popGraph:
        if options.compare_raw or options.compare_fitness:
            parser.error(
                "You can't use this graph type with only one identify !")
        ...
        if options.genrange:
            ...
            ret = c.execute(
                "select * from statistics where identify = ?", (options.identify,))
        ...
        for item in identify_list:
            if options.genrange:
                genrange = options.genrange.split(":")
                ret = c.execute(
                    "select * from statistics where identify = ? and generation between ? and ?", (item, genrange[0], genrange[1]))
            else:
                ret = c.execute(
                    "select * from statistics where identify = ?", (item,))
    ...
    if options.errorbars_fitness:
        if options.outfile:
            graph_errorbars_fitness(
                pop, options.minimize, options.outfile + "." + options.extension)
        else:
    ...
    if options.compare_fitness:
        if options.outfile:
            graph_compare_fitness(
                pop, options.minimize, identify_list, options.outfile + "." + options.extension)
        else:
            graph_compare_fitness(pop, options.minimize, identify_list)

    if options.pop_heatmap_raw:
        if options.outfile:
            graph_pop_heatmap_raw(
                pop, options.minimize, options.colormap, options.outfile + "." + options.extension)
        else:
            graph_pop_heatmap_raw(pop, options.minimize, options.colormap)

    if options.pop_heatmap_fitness:
        if options.outfile:
            graph_pop_heatmap_fitness(
                pop, options.minimize, options.colormap, options.outfile + "." + options.extension)
        else:
